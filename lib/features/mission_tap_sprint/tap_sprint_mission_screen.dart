...
class _TapSprintMissionScreenState extends ConsumerState<TapSprintMissionScreen> with WidgetsBindingObserver {
  ...
  Future<void> _playAlarm() async {
    ...
    try {
      if (alarm.isSoundEnabled) {
        ...
        debugPrint('[TapSprintMission] Playing alarm sound: $path');
        ...
        if (path == 'default' || path.isEmpty) {
          ...
          try {
            ...
            try {
              ...
              debugPrint('[TapSprintMission] AssetSource failed, trying BytesSource fallback: $e');
            ...
            debugPrint('[TapSprintMission] AudioPlayer error: $ae. Falling back to system default.');
            await FlutterRingtonePlayer().playAlarm(
              looping: true, 
              volume: alarm.volume, 
              asAlarm: true
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused) {
      ...
      debugPrint('[TapSprintMissionScreen] App paused at: $_backgroundTime');
    } else if (state == AppLifecycleState.resumed) {
      if (_backgroundTime != null) {
        final now = DateTime.now();
        final diff = now.difference(_backgroundTime!).inMinutes;
        debugPrint('[TapSprintMissionScreen] App resumed. Background duration: $diff minutes');
        
        if (diff >= 15) {
          debugPrint('[TapSprintMissionScreen] Background duration exceeds 15 minutes. Closing mission.');
          if (mounted) {
    ...
    try {
      ...
      if (_isSuccess) {
        try {
          ...
          await box.flush();
          debugPrint('[TapSprintMission] Hive state cleared successfully');
        } catch (e) {
          debugPrint('[TapSprintMission] Error clearing Hive state: $e');
        }
      }
    } catch (e) {
