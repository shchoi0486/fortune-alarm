...
class MLService {
  ...
  Future<MissionValidationResult> validateMissionLabels({
    ...
    try {
      ...
      // 3. Center Crop 2 (0.6) 분석 및 벡터 추출
        ...
        if (referenceFile != null) {
          if (_cachedReferencePath != referenceFile.path || _cachedReferenceVector == null) {
            ...
            if (refImage != null) {
              ...
              // 참조 이미지 벡터 추출 (우선순위: EfficientNet -> MobileNet)
              List<double> refVector = [];
              if (_efficientNetInterpreter != null) {
                refVector = _runEfficientNetVector(refImage);
              }
              if (refVector.isEmpty) {
                _runInference(refImage); // MobileNet 실행
                refVector = List.from(_lastInferenceVector);
              }
              _cachedReferenceVector = refVector;
              
              // 색상 히스토그램 캐싱
              _cachedReferenceHistogram = _calculateColorHistogram(refImage);
            }
          }
          ...
          if (_cachedReferenceVector != null && _cachedReferenceVector!.isNotEmpty) {
            ...
            simEff = math.max(s1, math.max(s2, s3));
          }
          ...
          // 하이브리드 점수 계산 개선
          // EfficientNet이 있으면 형태 60%, 색상 40% (색상 비중 상향)
          // EfficientNet이 없으면(simEff=0) 색상 90% (Fallback)
          double finalScore = 0.0;
          if (simEff > 0) {
            finalScore = (simEff * 0.6) + (colorSim * 0.4);
          } else {
            // EfficientNet 실패 시 색상에 거의 전적으로 의존
            finalScore = colorSim * 0.9; 
          }
          ...
          // 1. 라벨 일치
          // 2. 최종 점수가 0.75 이상 (기준 상향: 0.6 -> 0.75)
          // 3. 최종 점수가 0.85 이상이면 라벨 불일치 무시 (기준 상향: 0.8 -> 0.85)
          // 4. 색상 유사도가 매우 높고(0.9 이상) 형태 유사도도 어느정도(0.5 이상) 있으면 성공
          
          final bool highColorMatch = (colorSim >= 0.9 && simEff >= 0.5);
          
          // 더 엄격한 성공 조건: 라벨이 일치하더라도 최소한의 유사도(0.6)는 있어야 함
          final bool strictLabelMatch = labelMatch && maxSimilarity >= 0.6;
          
          final bool successCandidate = (strictLabelMatch || maxSimilarity >= 0.75 || highColorMatch);
          
          if (successCandidate) {
            _successStreak += 1;
          } else {
            // 연속 성공이 끊기면 바로 0이 아니라 1 감소 (노이즈 방지) - 선택적 적용
            // 여기서는 엄격함을 위해 0으로 초기화
            _successStreak = 0;
          }
          
          // 연속 3회 이상 성공해야 최종 인정 (약 1.5초~2초 유지 필요)
          if (_successStreak >= 3) {
        ...
        if (missionType == MissionType.cameraSink) {
          ...
          for (final prediction in predictions) {
           ...
           for (final prediction in predictions) {
             final label = (prediction['label'] as String).toLowerCase();
             final confidence = prediction['confidence'] as double;
             if (confidence >= 0.5 && label.contains(targetKeyword.toLowerCase())) {
               return MissionValidationResult(
                 isSuccess: true, 
